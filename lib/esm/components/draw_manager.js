    ...
    var lineLastMarkerHandlers = React.useMemo(function () { return ({
        click: function () {
            if (!initFirstMarker) {
                ...
                if (polygonCreate) {
                    props.setData({
                        points: __spreadArray(__spreadArray([], polygonModel), [polygonModel[0]]).map(function (bound) { return destructBoundTuple(bound); })
                    });
                    return true;
                }
                props.setData({
                    points: polylineModel.map(function (bound) { return destructBoundTuple(bound); })
                });
                return true;
            }
            return false;
