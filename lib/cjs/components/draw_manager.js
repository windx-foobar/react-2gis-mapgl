    ...
    var lineLastMarkerHandlers = react_1.default.useMemo(function () { return ({
        click: function () {
            if (!initFirstMarker) {
                ...
                if (polygonCreate) {
                    props.setData({
                        points: tslib_1.__spreadArray(tslib_1.__spreadArray([], polygonModel), [polygonModel[0]]).map(function (bound) { return helpers_1.destructBoundTuple(bound); })
                    });
                    return true;
                }
                props.setData({
                    points: polylineModel.map(function (bound) { return helpers_1.destructBoundTuple(bound); })
                });
                return true;
            }
            return false;
